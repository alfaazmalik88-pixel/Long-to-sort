...
async function test() {
  try {
    ...
    for (const inst of instances) {
      ...
      try {
        const vidRes = await fetch(inst.uri + "/api/v1/videos/jNQXAC9IVRw", { signal: AbortSignal.timeout(3000) });
        if (vidRes.ok) {
          const vidData = await vidRes.json();
          if (vidData.formatStreams && vidData.formatStreams.length > 0) {
            console.log("Success with", inst.uri, vidData.formatStreams[0].url.substring(0, 50));
            return;
          }
        }
      } catch(e) {
